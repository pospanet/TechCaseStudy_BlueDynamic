# Workflow 2 — Detekce a odstranění antipatternů
...
| Review business zadání | GPT-5.4 | `review-change` nebo bez skillu | nový nezávislý |
...
| Review plánu | GPT-5.4 | `review-change` | nový nezávislý |
| Zapracování připomínek | Claude Opus 4.6 | `explore-repository` | původní |
| Atomické tasky | Claude Sonnet 4.6 | `implement-task` | nový |
| Implementace tasku | Claude Sonnet 4.6 | `implement-task` | samostatný pro task |
| Code review | GPT-5.4 | `review-change` | nový nezávislý |
| Oprava nálezů | Claude Sonnet 4.6 | `implement-task` | původní implementační |
| Finální validace | GPT-5.4 | `validate-project` | nový |
...
Code and architecture antipattern remediation
...
- snížení nákladů na údržbu a delivery risk,
- lepší změnitelnost a testovatelnost,
- odstranění nejškodlivějších návrhových a kódových antipatternů,
- zachování externě pozorovatelného chování,
- prioritizaci podle value-to-effort.
...
- cílové antipatterny jsou odstraněny nebo podstatně omezeny,
- externě pozorovatelné chování se nezmění,
- rizikové cesty jsou pokryty characterization nebo regression testy,
- dependency direction se zlepší,
- duplicity jsou odstraněny bez vzniku nové god abstraction.

Avoid implementation design unless it is a necessary business constraint.

Save as:
`docs/work-items/antipattern-remediation-business-requirement.md`
...
`docs/work-items/antipattern-remediation-business-requirement.review.md`
...
- duplicity,
- static state a hidden dependencies,
- míchání business a infrastruktury,
- async misuse,
- nekonzistentní error handling,
- testability barriers a dead code.

Every mandatory acceptance criterion must map to implementation steps, validation steps and expected evidence.

Save as:
`docs/work-items/antipattern-remediation-implementation-plan.md`
...
Save the complete review as:
`docs/work-items/antipattern-remediation-implementation-plan.review.md`
...
Save as:
`docs/work-items/antipattern-remediation-tasks.md`
...
Save the complete review as:
`docs/work-items/antipattern-remediation-TASK-ID-code-review.md`

For a reviewed task range, use:
`docs/work-items/antipattern-remediation-TASK-ID-to-TASK-ID-code-review.md`
...
- single task: `docs/work-items/antipattern-remediation-TASK-ID-code-review.md`
- task range: `docs/work-items/antipattern-remediation-TASK-ID-to-TASK-ID-code-review.md`
...
7. Update the affected task entries in `docs/work-items/antipattern-remediation-tasks.md` with final status, validation evidence and any approved residual risk.
