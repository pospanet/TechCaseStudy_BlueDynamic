...
Library
/
devin-code-workflow-cheat-sheet.md


# Devin Desktop – Workflow Cheat Sheet pro práci nad kódem
...
1. **Review prováděj v nové session.**
2. Reviewerovi dej požadavky, diff a relevantní kontext, ale nevnucuj mu závěry implementátora.
3. **Nepoužívej stejný model jako jediného autora i jediného reviewera** u high-risk změn.
4. Levný model používej na mechanickou práci a validaci, nikoli jako jediný zdroj architektonického rozhodnutí.
5. Finální report musí rozlišovat:
   - co bylo skutečně ověřeno,
   - co je inference,
   - co nebylo možné ověřit.
6. Procházej celý výsledný diff, ne pouze poslední opravené soubory.
7. Testy jsou důkazem pouze toho, co skutečně pokrývají.
Library
/
devin-code-workflow-cheat-sheet.md
